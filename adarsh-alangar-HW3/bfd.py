...
class BF:
        ...
        with open(config.config_file, 'r') as f:
            ...
            for item in j["neighbours"]:
                ...
                if cost != self.dv[ip][config.COST]:
                    print ("Change detected at: ", datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3])
                    change = True
                    self.dv[ip] = (ip, cost)
        return change
    ...
    def print_routing_table(self):
        ...
        """
        table = []
        header = ["Destination Network", "Next Hop", "Next Hop IP", "Cost"]
        # Add immediate neighbours to the routing table
        for node, ip in self.neighbour_ip.items():
            # [IP, NextHOP, NextHOP ID,COST]
            table.append([ip, "Direct", "-", self.dv[ip][config.COST]])
        neighbour_ip = self.neighbour_ip.values()
        for dest in self.dv:
            if dest in neighbour_ip or dest in self.interfaces or dest == "id":
                # Already considered in the previous loop, so ignore
                # Or it's me, so ignore
                # Or it's my ID field
                continue
            next_hop, cost = self.dv[dest]
            table.append([dest, next_hop, self.neighbour_ip[next_hop], cost])
        print (datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3])
        print(tabulate(table, headers=header))

    def run(self):
        # Run infinitely.
        print ("Server up at: ", datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3])
        while True:
            change = False
            try:
